...
public class MinimumCostPath {
	...
	//minimum cost path from cell(i,j) to cell(m-1,n-1)
	...
	public static int minCostPathDP(int[][] arr) {
		...
		//start filling rest of every row
		for(int i=m-2; i>=0; i--) {
			for(int j=n-2; j>=0; j--) {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		System.out.println(MinimumCostPath.minCostPathDP(arr));
		System.out.println(MinimumCostPath.minCostPathM(arr));
		System.out.println(MinimumCostPath.minCostPath(arr));
	}
